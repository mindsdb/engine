...
class TestMossHandlerConnection(unittest.TestCase):
    ...
    @patch("mindsdb.integrations.handlers.moss_handler.moss_handler.MossClient")
    def test_check_connection_success(self, _mock_client, mock_run):
    ...
    @patch("mindsdb.integrations.handlers.moss_handler.moss_handler._run")
    @patch("mindsdb.integrations.handlers.moss_handler.moss_handler.MossClient")
    def test_check_connection_failure(self, _mock_client, mock_run):
...
class TestMossHandlerInsert(unittest.TestCase):
    ...
    @staticmethod
    def _run_side_effects(_mock_run):
    ...
    def test_insert_creates_index_when_new(self, mock_run):
        ...
        # list_indexes → [], create_index → None, get_index → Ready, load_index → None
        ready = _mock_index(status="Ready")
        mock_run.side_effect = [[], None, ready, None]
    ...
    def test_insert_upserts_when_index_exists(self, mock_run):
        ...
        ready = _mock_index(status="Ready")
        # list_indexes → [existing], add_docs → None, get_index → Ready, load_index → None
        mock_run.side_effect = [[existing], None, ready, None]
